...
export function SearchPalette({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal
      label="Search or ask Amiva"
      position="top"
      onClose={onClose}
      panelClassName="w-full max-w-160"
    >
        ...
        <div className="max-h-[46vh] overflow-y-auto p-4" aria-live="polite">
          {searching ? (
            <div className="flex items-center gap-3 py-6 text-sm text-ink-muted">
              <Sparkles
                className="size-4 animate-pulse text-cyan-600"
                aria-hidden
              />
                    ...
                    {result.citations.map((c, i) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex w-full cursor-pointer items-center gap-3 rounded-control border border-line px-3 py-2.5 text-left hover:border-indigo-300"
                        >
                          <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-indigo-50">
                            <Icon
                              className="size-4 text-indigo-900"
                              aria-hidden
                            />
                          </span>
