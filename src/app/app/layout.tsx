    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-soft">
        <Image
          ...
          width={40}
          height={40}
          className="animate-pulse rounded-[22%]"
        />
      </div>
    );
  }
      ...
      <div className="flex min-w-0 flex-1 flex-col md:pl-60">
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-line bg-white/85 px-4 backdrop-blur md:px-6">
          <button
            ...
            className="flex size-9 shrink-0 cursor-pointer items-center justify-center rounded-control text-ink-muted hover:bg-indigo-50 md:hidden"
          >
            <Menu className="size-5" aria-hidden />
          </button>
          <button
            onClick={() => setSearchOpen(true)}
            className="flex h-9 w-full max-w-105 cursor-pointer items-center gap-2 rounded-control border border-line bg-soft px-3 text-sm text-ink-muted hover:border-indigo-300"
          >
            <Search className="size-4" aria-hidden />
            <span className="sm:hidden">Search</span>
            <span className="hidden sm:inline">Search or ask Amiva…</span>
          ...
          <div className="ml-auto flex items-center gap-1.5">
            <button
              ...
              className="relative flex size-9 cursor-pointer items-center justify-center rounded-control text-ink-muted hover:bg-indigo-50"
            ...
            <div className="flex-1 divide-y divide-line overflow-y-auto">
              {notifications.map((n) => (
                <Link
                  ...
                  className={cn(
                    "block px-5 py-3.5 hover:bg-soft",
                ...
                >
                  <p className="flex items-center gap-2 text-sm font-medium text-navy">
                    {!n.read && (
                      <span
                        className="size-1.5 rounded-full bg-cyan-600"
                        aria-label="Unread"
                      />
                    )}
                    {n.title}
        ...
        >
          <Card className="p-6">
            ...
            <p className="mt-2 text-sm leading-relaxed text-ink-muted">
              You&apos;ll need to sign in again to see your reminders, tasks and
              calendar here. WhatsApp keeps working as usual.
            </p>
