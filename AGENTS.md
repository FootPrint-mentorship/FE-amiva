...
> **Session note (24 Aug 2026, night) — WhatsApp connect flow + Sentry FE + endpoint audit (STAGED uncommitted, tsc + 144 vitest + lint + prod build green):** (1) **Web-initiated WhatsApp linking finally wired** (Settings→Integrations Connect used to `router.push("/link")` with no token → "Link expired" dead end): Connect now calls `POST /link/whatsapp/code` (`createLinkCode` in data/linking.ts) → modal with the wa.me deep link + fallback code (**must show `LINK <code>` — the bot's `LINK_CODE_RE` rejects a bare code**) → 4s poll of loadMe() until `whatsapp_linked` flips or `expires_at` passes (expired state offers a fresh code). `/app/settings?connect=whatsapp` auto-opens the flow (same deferred idiom as `?connected=google`); `/link` without a token now offers "Connect WhatsApp" when authed. **Unlink was a local-only lie** (flipped the store, never hit the server) — now calls `DELETE /link/whatsapp` (`unlinkWhatsApp`). Browser-verified end-to-end vs local BE (real minted code AMV-…, wa.me href, waiting spinner). (2) **NFR-04 fix**: the recurrence chip made /app/reminders h-scroll at 375px — chip now takes `min-w-0 max-w-full` + a truncating text span (full text in title attr, Today page and the edit modal); page stays 375/375. Mobile row density (time gutter squeezing the content column, title truncates early) is a pre-existing design item for later. (3) **Memories "Export all" exported only the cached first page (50)** — now `apiBlob("/memories/export")` (full server stream) + saveBlob. (4) **@sentry/nextjs 10.70 wired, DSN-gated**: `src/instrumentation-client.ts` (error-only Replay, all text/media masked) + `src/instrumentation.ts` (server/edge + `onRequestError`) + `src/app/global-error.tsx` (last-resort boundary) + `withSentryConfig` in next.config.ts (source maps upload only when SENTRY_AUTH_TOKEN present). PII scrubbers in `src/lib/sentry-scrub.ts` mirror the BE (emails/numbers/JIDs; request data dropped). Needs on Netlify: `NEXT_PUBLIC_SENTRY_DSN` (new Sentry project "amiva-web") + optional `SENTRY_ORG`/`SENTRY_PROJECT`/`SENTRY_AUTH_TOKEN` for readable stacks. (5) **Endpoint-coverage audit vs openapi.yaml (68 paths)** — all wired except (deliberate/flagged): `/calendar/calendars` + `/calendar/calendars/selection` (NO web UI to pick synced calendars — biggest real gap), `/search/sources` GET/PUT (palette chips are per-session, server persistence unused → toggles reset per device), `/reminders/{id}/deliveries` (delivery trail not surfaced), `/tasks/projects` (free-text field, no autocomplete), `/memories/categories` (client-side counts — wrong past 50 items, same class as the export bug), `/calendar/agenda`+`/availability` (chat-only by design), `/meta/timezones` (Intl by design).
>
> **Session note (24 Aug 2026) — SEO + a11y + perf batch (STAGED uncommitted, tsc + 136 vitest + prod build green):** (1) **Indexing**: new `app/robots.ts` (marketing allowed; /app/, /onboarding and the whole auth funnel disallowed) + `app/sitemap.ts` (/, /terms, /privacy-policy); noindex metadata on the (public) group and on /app via a NEW server `app/app/layout.tsx` that wraps the old client layout (renamed `app-shell.tsx`, exports `AppShell`). (2) **SEO**: canonicals on root/terms/privacy; `public/og.png` (1200×630 brand card, PIL-generated) wired as og:image + twitter summary_large_image; FAQPage JSON-LD on the home page fed from the SAME data as the accordion — `faqs` moved to `faq-data.ts` (plain module; importing it from the client faq.tsx into the server page yielded a client-reference proxy, not the array). (3) **A11y (Lighthouse 94 → both named fails fixed)**: `--color-ink-muted` #6b6f80→#646880 (was 4.43:1 on the lavender cards; now 4.87 lavender / 5.48 white); hero floating-card `h4`s → styled `p`s (H1→H4 skip); the DashboardPreview mockup is `aria-hidden` with its `main`/`aside`/`h3` demoted to divs/p (it rendered TWICE → 3 mains on the page); skip-to-content link; hamburger got aria-expanded/aria-controls + Escape closes the drawer; FAQ buttons got aria-controls/panel ids. Audit after: 0 contrast fails, valid heading order, exactly one header/nav/main/footer. (4) **Perf (mobile 93)**: marketing layout split — server layout + `components/marketing/header.tsx` client island (the only stateful part), footer is now static HTML; `browserslist` added (modern evergreen — drops legacy polyfills); hero `Image` got a `sizes` hint. Remaining Lighthouse items (render-blocking CSS, network tree) are Netlify-delivery-side — re-run Lighthouse after deploy. NB the dev-only hydration warning about the `js` class is the long-standing crawler-safe reveal gate, not a regression.
>
> **Session note (23 Aug 2026) — prod QA fixes + email assets (STAGED uncommitted, tsc + 136 vitest green):** (1) **REM-13 fixed** — `Reminder.due_at` is now `string | null` (matching the contract; recurring reminders have none) and the reminders list groups/renders by `due_at ?? next_fire_at ?? snoozed_until` — prod was showing recurring rows as "Thu 1 Jan, 1:00 am WAT" (Unix epoch). Fixtures corrected (recurring rows carried fake due_at, which is why no test caught it) + regression test. (2) **Reminder delete had TWO prod bugs** ("the deleting issue"): the row menu never flips, so near the viewport bottom Pause/Delete sat below the fold unreachable — it now measures the trigger and opens upward (`menuFlipsUp`); and Delete executed with NO confirmation — it now opens a confirm modal (Keep it / Delete reminder, recurring-aware copy). Both browser-verified locally. (3) **TASK-13**: task drawer's Source line was the hardcoded mock string "Created via WhatsApp · today" — now renders real `task.source` + `created_at` (type extended; backend always returned them). (4) **public/email/** added: mark.png + 4 hero images (from the 17 Aug design PNGs; deletion hero cropped from its mockup) served at tryamiva.com/email/* for the backend's new HTML transactional emails — **deploy this repo with/before the backend email change** or emails render alt text.
>
