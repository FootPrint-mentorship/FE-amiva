...
- The query cache is in-memory: state persists across navigation but not across a full reload (real mode refetches; mock mode reseeds).
...
- Search answers are canned in mock mode; drafts are template-generated. Subtask suggestions now call the real `POST /tasks/{id}/suggest-subtasks` in real mode (16 Aug) — the backend needs an LLM key configured or it answers 502 PROVIDER_ERROR (honest toast; mock mode keeps canned ideas).
